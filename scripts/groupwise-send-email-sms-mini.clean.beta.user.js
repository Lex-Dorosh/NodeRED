...
// @grant        none
// @noframes     false
// @downloadURL  https://raw.githubusercontent.com/Lex-Dorosh/NodeRED/gxo-beta/scripts/groupwise-send-email-sms-mini.clean.beta.user.js
// @updateURL    https://raw.githubusercontent.com/Lex-Dorosh/NodeRED/gxo-beta/scripts/groupwise-send-email-sms-mini.clean.beta.user.js
// ==/UserScript==
...
(function () {
  ...
  async function sendEmailOnly() {
    const mailBtn = findEmailOwnerButton();
    if (!mailBtn) {
      alert('SEND_EMAIL: owner email button not found (Bericht eig.)');
      return false;
    }
  ...
  async function sendSmsOnly() {
    ...
    log('SEND_SMS done via remote popup handler');
    return true;
  }
  ...
  async function sendBoth(btn) {
    ...
    try {
      ...
      await sleep(300);
      const okSms = await sendSmsOnly();
      log(`Done. SEND_EMAIL=${okEmail ? 'OK' : 'FAIL'}, SEND_SMS=${okSms ? 'OK' : 'FAIL'}`);
    } finally {
      btn.disabled = false;
      btn.textContent = old;
    }
