...
// @version      2.94.0
// @description  Cleans side buttons panel: hide/keep rules + config UI (+/−), import/export, stable across frames.
...
(function () {
  ...
  const resetOverrides = () => savePrefs({ forceHide: {}, forceKeep: {} });
  ...
  function injectCss(doc){
    ...
    const css = `
      .uic-panel{border:1px solid #e5e7eb;border-radius:10px;padding:8px 10px;background:#ffffff;margin:8px 0 10px 6px;}
      .uic-panel h4{margin:0 0 6px 0;font-size:14px;font-weight:700;color:#111827;}
      .uic-row{display:flex;gap:6px;flex-wrap:wrap;align-items:center}
      .uic-row .button{border:1px solid #d0d5dd !important;border-radius:6px !important;background:#f3f4f6 !important;color:#111827 !important;padding:4px 6px !important;height:24px !important;}
      .${HIDDEN_CLASS}{display:none !important;}
      #${TOGGLE_ID}, #${CONFIG_ID}{display:none;margin:4px 6px 6px 6px;padding:6px 10px;border:1px solid #ced4da;border-radius:10px;background:#f8f9fa;color:#212529;cursor:pointer;font-size:12px;position:relative;z-index:2147483646;}
      #${CONFIG_ID}{width:32px;height:32px;padding:0;display:none;align-items:center;justify-content:center;font-size:16px;font-weight:900;background:#fff7ed;border-color:#fed7aa;}
      #${CONFIG_PANEL_ID}{display:none;margin:4px 6px 6px 6px;padding:6px 8px;border:1px dashed #cbd5e1;border-radius:12px;background:#ffffff;}
      ...
      #${CONFIG_PANEL_ID} .uic-iconbtn{display:inline-flex;align-items:center;justify-content:center;width:28px;height:28px;border-radius:10px;border:1px solid #d0d5dd;background:#f8fafc;cursor:pointer;user-select:none;font-size:14px;padding:0;}
      ...
      .uic-item{display:inline-flex;align-items:center;gap:4px;margin:0 6px 6px 0;padding:2px 4px;border-radius:10px;border:1px solid transparent;vertical-align:middle;}
      ...
      .uic-ctl button{width:20px;height:20px;line-height:18px;border-radius:999px;border:1px solid #cbd5e1;background:#fff;cursor:pointer;font-weight:900;font-size:12px;padding:0;}
  ...
  function groupCustomerOwner(doc, side){
    ...
    row.className = "uic-row";
    panel.appendChild(row);

    [cust, own].forEach(span=>{
      if (!span) return;
      const b = span.querySelector("b");
      if (b){
        const lbl = doc.createElement("span");
        lbl.textContent = b.textContent.trim();
        lbl.style.cssText = "font-weight:700;margin-right:6px;color:#111827;";
        row.appendChild(lbl);
      }
      Array.from(span.querySelectorAll("input.button")).forEach(btn => row.appendChild(btn));
      span.style.display = "none";
    });

    const bereken = Array.from(side.querySelectorAll("input.button")).find(b => BEREKEN_VALUE.test(b.value || ""));
    if (bereken){
      side.insertBefore(panel, bereken);
      bereken.style.marginTop = "6px";
    } else {
    ...
    if (!panel){
      ...
      panel.innerHTML = `
        <div class="uic-cfgbar">
          ...
          <span class="uic-legend" title="Legend">+ keep • − hide</span>
        </div>
      `;
